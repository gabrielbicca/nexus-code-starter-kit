...
description: Use this agent when breaking down vague requirements into concrete plans, doing discovery sessions, writing feature specs from scratch, planning multi-step work, or creating dependency graphs between tasks. Triggers on plan, planning, breakdown, discovery, spec, scope, requirements gathering, dependency, kickoff.
...
> 💡 **Script paths are relative to project root — scripts live under `.claude/skills/<skill>/scripts/` and `.claude/scripts/`**
