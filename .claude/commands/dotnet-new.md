---
description: Gera a estrutura de um projeto backend .NET novo a partir de um padrão — clean (Clean Architecture + DDD, .NET 10 LTS) ou ntier (N-Tier em camadas, .NET 8 LTS). Sempre usa versões LTS. Use ao iniciar um projeto/solução dotnet do zero ou adicionar um portal/serviço.
---

# /dotnet-new — Scaffold de projeto .NET (clean | ntier)
...
2. **Só versões LTS.** Nunca scaffolde em STS. `clean` → .NET 10 (LTS); `ntier` → .NET 8 (LTS). ORMs acompanham a major do framework. Se o usuário pedir explicitamente outra versão LTS, confirme.
...
```
/dotnet-new <nome> [--pattern clean|ntier] [--orm efcore|dapper|both] [--portal <nome>]
```

- `<nome>` — nome da solução/projeto (PascalCase). Vira o prefixo (`<Nome>.Api`, `<Nome>.WEB`, ...).
- `--pattern` — `clean` (default) ou `ntier`. Se ausente e ambíguo, **pergunte** antes de gerar.
- `--orm` — `efcore` (escritas), `dapper` (leituras), `both` (default no `clean`: EF write + Dapper read).
- `--portal` — só no `ntier`: cria um portal web adicional (`<Nome>.<Portal>.WEB`) referenciando as camadas de backend existentes.
...
2. **Resolver o padrão**
   ...
   - Ausente → faça 1-2 perguntas (greenfield DDD/microserviço? → `clean`; encaixar em solução N-Tier/MVC multi-portal? → `ntier`).
3. **Fixar versões LTS** — `clean`: net10.0; `ntier`: net8.0. Pacotes EF Core na mesma major.
...
7. **`--portal`** (ntier) — pular criação de camadas; criar só o novo `.WEB` e referenciar BUSINESS/DATA/ENTITY/LANGUAGE.
...
```
[OK] Projeto <Nome> gerado — padrão <clean|ntier>, .NET <versão LTS>.
...
Próximos passos:
- /adr registrar a escolha de arquitetura (<clean|ntier>)
...
/dotnet-new NEWPORTAL --pattern ntier
/dotnet-new Relatorios --pattern ntier
```
